...
class SqlOperator(object):
    ...
    def get_list_from_sql(self, table):
        with closing(self.get_conn()) as conn:
            with closing(conn.cursor(pymysql.cursors.DictCursor)) as cur:
                cur.execute("select * from %s " % (table))
                return cur.fetchall()

    def get_list_from_sql_by_regular(self, table, key, key_value):
        with closing(self.get_conn()) as conn:
            with closing(conn.cursor(pymysql.cursors.DictCursor)) as cur:
                # value = '%'+self.date+'/%%'
                sqlOrder = f"select * from {table} where {key} like '%{key_value}%' "
                print(f"mySql quering sqlOrder using:{sqlOrder}")
                cur.execute(sqlOrder)
                return cur.fetchall()
    def get_list_sql_by_column(self, table, listColumn):
        with closing(self.get_conn()) as conn:
            with closing(conn.cursor(pymysql.cursors.DictCursor)) as cur:
                strlistColumn = ','.join(listColumn)
                sqlOrder = f"select {strlistColumn} from {table} "
                print(f"mySql quering sqlOrder using:{sqlOrder}")
                cur.execute(sqlOrder)
                return cur.fetchall()
